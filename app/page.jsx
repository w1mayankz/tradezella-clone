...
export default function App() {
  ...
  // Create interleaved array for perfect grid alignment
  const interleavedCalendar = [];
  for (let i = 0; i < 6; i++) {
    interleavedCalendar.push(...calendarDays.slice(i * 7, i * 7 + 7));
    interleavedCalendar.push({ isWeeklyStat: true, ...weeklyStats[i] });
  }
            ...
            <div className="max-w-[1920px] mx-auto space-y-4 md:space-y-6">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 md:gap-6">
                ...
                <div className="xl:col-span-8 flex flex-col">
                  <div className="bg-[#13131a] rounded-xl border border-[#22222a] p-5 flex flex-col h-full shadow-md">
                    
                    {/* Calendar Header */}
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-1">
                          <button className="p-1 hover:bg-[#22222a] rounded transition-colors text-gray-400"><ChevronLeft size={18}/></button>
                          <span className="text-[15px] font-bold text-white px-2">June 2024</span>
                          <button className="p-1 hover:bg-[#22222a] rounded transition-colors text-gray-400"><ChevronRight size={18}/></button>
                        </div>
                        <button className="bg-[#1a1a24] border border-[#2c2c35] px-3 py-1.5 text-[13px] font-medium text-gray-300 rounded-md hover:bg-[#22222a] transition-colors">This month</button>
                      </div>
                      <div className="flex items-center gap-3 text-[13px]">
                        <span className="text-gray-400 font-medium">Monthly stats:</span>
                        <span className="bg-[#21ce99] text-[#0b0b0f] font-bold px-2.5 py-0.5 rounded-full">$5.13K</span>
                        <span className="bg-[#2e44ff] text-white font-bold px-2.5 py-0.5 rounded-full">13 days</span>
                        <div className="flex items-center gap-3 text-gray-400 ml-3">
                           <Settings size={16} className="hover:text-gray-200 cursor-pointer transition-colors" />
                           <Camera size={16} className="hover:text-gray-200 cursor-pointer transition-colors" />
                           <Info size={16} className="hover:text-gray-200 cursor-pointer transition-colors" />
                        </div>
                      </div>
                    </div>

                    {/* Calendar Grid Container (8 Columns: 7 Days + 1 Weekly Stat) */}
                    <div className="flex flex-col flex-1 overflow-x-auto hide-scrollbar">
                      <div className="min-w-[700px]">
                        
                        {/* Headers */}
                        <div className="grid grid-cols-[repeat(7,1fr)_90px] mb-2">
                          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', ''].map((day, i) => (
                            <div key={i} className="text-center text-[12px] font-medium text-gray-500 pb-1">{day}</div>
                          ))}
                        </div>

                        {/* Grid Body */}
                        <div className="grid grid-cols-[repeat(7,1fr)_90px] gap-[1px] bg-[#22222a] border border-[#22222a] rounded-lg overflow-hidden shadow-inner">
                          {interleavedCalendar.map((item, i) => {
                            if (item.isWeeklyStat) {
                              return (
                                <div key={`week-${i}`} className="bg-[#13131a] p-3 flex flex-col items-start justify-center pl-4 relative group">
                                  <span className="text-[11px] font-medium text-gray-500 mb-1">{item.week}</span>
                                  <span className="text-[14px] font-bold text-white">{item.pnl}</span>
                                  <div className="mt-1.5 bg-[rgba(131,105,255,0.1)] text-[#8369ff] text-[10px] px-2 py-0.5 rounded font-semibold border border-[rgba(131,105,255,0.2)]">
                                    {item.days}
                                  </div>
                                </div>
                              );
                            } else {
                              return <CalendarCell key={`day-${i}`} data={item} />;
                            }
                          })}
                        </div>
...
function CalendarCell({ data }) {
  ...
  // Exact color matching from TradeZella
  let bgClass = 'bg-[#1b1b22] hover:bg-[#202028]'; // Default empty current month
  if (!isCurrentMonth) bgClass = 'bg-[#13131a] opacity-60'; // Empty other month
  
  if (cellData) {
    switch(cellData.type) {
      case 'win-bright': 
      case 'win-dark': 
        bgClass = 'bg-[#133224] hover:bg-[#183e2c]'; 
        break;
      case 'loss': 
        bgClass = 'bg-[#3f1d1d] hover:bg-[#4a2222]'; 
        break;
      case 'neutral': 
        bgClass = 'bg-[#1c2237] hover:bg-[#232a45]'; 
        break;
      default: break;
    }
  }
  ...
  return (
    <div className={`relative flex flex-col justify-between p-2.5 min-h-[110px] transition-colors cursor-pointer group ${bgClass}`}>
      <div className="flex justify-between items-start w-full">
        <div className="text-gray-400 opacity-70 group-hover:opacity-100 transition-opacity">
           {cellData?.icon && <FileText size={13} strokeWidth={2.5} />}
        </div>
        <span className={`text-[12px] font-medium ${isCurrentMonth ? 'text-gray-400' : 'text-gray-600'}`}>{day}</span>
      </div>
      
      {cellData && (
        <div className="flex flex-col items-center text-center mt-1">
          <span className="text-[14px] font-bold text-white tracking-tight">{cellData.pnl}</span>
          <span className="text-[10px] font-medium text-gray-400 mt-1">{cellData.trades} trade{cellData.trades > 1 ? 's' : ''}</span>
          <span className="text-[10px] font-medium text-gray-400">{cellData.win}</span>
        </div>
      )}
    </div>
  );
